...
adams({ 
  ...
  if (!ms.message?.extendedTextMessage?.contextInfo?.quotedMessage?.documentMessage && 
      !ms.message?.extendedTextMessage?.contextInfo?.quotedMessage?.documentWithCaptionMessage) {
    return repondre({
  ...
  try {
    ...
    const vcfData = await downloadMedia(quotedMessage);
